...
_ASSET_ROOT = files("mjlabplusplus") / "assets" / "robots" / "unitree_go2"
UNITREE_GO2_XML: Path = Path(str(_ASSET_ROOT / "mjcf" / "go2.xml"))
UNITREE_GO2_URDF: Path = Path(str(_ASSET_ROOT / "urdf" / "go2_description.urdf"))
...
UNITREE_GO2_FOOT_GEOM_NAMES = tuple(
    f"{site}_collision" for site in UNITREE_GO2_FOOT_SITE_NAMES
)
...
def _name_collision_geoms(spec: mujoco.MjSpec) -> None:
    ...
    for body_name, site_name in zip(
        UNITREE_GO2_FOOT_BODY_NAMES, UNITREE_GO2_FOOT_SITE_NAMES, strict=True
    ):
...
def get_unitree_go2_robot_cfg() -> EntityCfg:
    """Get a fresh Unitree Go2 robot configuration instance."""
    return EntityCfg(
